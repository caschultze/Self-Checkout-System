...
//		int option = 3;
	...
	@Test
	public void testGiftCardInsert() throws IOException {
		Card card = new Card("Giftcard", "3003091283", "Carmen Sandiego", "555", "1234", true, true);
		Calendar expiry = Calendar.getInstance(TimeZone.getTimeZone("MDT"));
		expiry.add(Calendar.YEAR, 2);
		BigDecimal gcAmount = new BigDecimal("100.00");
		...
		boolean expected = true;
		boolean actual = false;
		control.giftcardPayment.insertGiftCard(card, "1234");
		int holdNumber = control.paymentProcessing.authorize("3003091283", totalPrice);
		actual = control.paymentProcessing.post("3003091283", holdNumber, totalPrice);
		assertEquals(expected, actual && control.giftcardPayment.getInsertCheck());
	}
	
	@Test (expected = NullPointerException.class)
	public void testGiftcardInsertNullCard() throws IOException {
		Card card = null;
		String pin = "1234";
		control.giftcardPayment.insertGiftCard(card,pin);
	}
	
	@Test (expected = NullPointerException.class)
	public void testGiftCardtInsertNullPin() throws IOException {
		Card card = new Card("Giftcard", "3003091283", "Carmen Sandiego", "555", "1234", true, true);
		String pin = null;
		control.giftcardPayment.insertGiftCard(card,pin);
	}
	...
	@Test (expected = SimulationException.class)
	public void testGiftCardInsertWrongType() throws IOException {
		Card card = new Card("Membership", "3003091283", "Carmen Sandiego", "555", "1234", true, true);
		Calendar expiry = Calendar.getInstance(TimeZone.getTimeZone("MDT"));
		expiry.add(Calendar.YEAR, 2);
		BigDecimal gcAmount = new BigDecimal("100.00");
	...
	@Test
	public void testCreditRemove() throws IOException {
		Card card = new Card("Giftcard", "3003091283", "Carmen Sandiego", "555", "1234", true, true);
		Calendar expiry = Calendar.getInstance(TimeZone.getTimeZone("MDT"));
		expiry.add(Calendar.YEAR, 2);
		BigDecimal gcAmount = new BigDecimal("100.00");
		...
		int holdNumber = control.paymentProcessing.authorize("3003091283", totalPrice);
		actual = control.paymentProcessing.post("3003091283", holdNumber, totalPrice);
		control.giftcardPayment.giftRemove();
		assertEquals(expected, control.giftcardPayment.removeCheck());
	}
